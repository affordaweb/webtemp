  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
        ...
        <a href="${siteUrl}/admin" style="display:inline-block;margin-top:24px;background:#5636D1;color:#fff;padding:12px 28px;border-radius:999px;font-weight:700;text-decoration:none">View in Admin</a>
      </div>
    `,
  })
}
...
export async function sendDownloadReady(req: TemplateRequest): Promise<void> {
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
        <div style="background:linear-gradient(135deg,#0F0F1A,#5636D1,#E2498A);padding:40px 32px;border-radius:16px;text-align:center;margin-bottom:32px">
          <h1 style="color:#fff;margin:0 0 8px;font-size:26px;font-weight:900">Your Template is Ready! 🎉</h1>
          <p style="color:rgba(255,255,255,0.8);margin:0;font-size:15px">Payment confirmed — your custom website template is waiting.</p>
        </div>
        <p style="color:#374151;font-size:15px">Hi ${req.name},</p>
        <p style="color:#374151;font-size:15px">Your website template for <strong>${req.businessName}</strong> has been generated and is ready to download. The link below is unique to your request and expires after 7 days.</p>
        <div style="text-align:center;margin:32px 0">
          <a href="${downloadUrl}" style="display:inline-block;background:#5636D1;color:#fff;padding:16px 40px;border-radius:999px;font-weight:800;text-decoration:none;font-size:16px">Download Your Template</a>
        </div>
        <p style="color:#6b7280;font-size:13px">Inside the zip you'll find your template files plus a step-by-step guide to get your site live. If you need any help, reply to this email.</p>
        <p style="color:#6b7280;font-size:13px;margin-top:24px">— The WebTemp Team</p>
      </div>
    `,
  })
}
